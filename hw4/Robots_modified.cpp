...
int Robot::getxcoord()//taking coordinate to modify
{
	return xPos;
}
int Robot::getycoord()//taking ycoordinate
{
	return yPos;
}

int Robot::getbagcount()//finding bagcounts
{
	return bag;
}
void Robot::Turnface(Direction dir)//turning face
{
	if (dir==east)
	{
		direction=east;
...
void Robot ::TurnRandom()//randomnumbers for monster movement
{
	RandGen ran;
	int randomn=ran.RandInt(1,4);
	if (randomn==1)
	{
		direction=east;
	}
...
bool Robot::isalive()//finding alive or not
{
	return !stalled;

}
void Robot::ressurect()//when some conditions arent provided then resurect monsters 
{
	stalled=false;
	theRobotWindow->Redraw(this);
}
void Robot::kill()//killing monster and hunter 
{
	stalled=true;
	theRobotWindow->Redraw(this);
}
void Robot::pickallthethings()//picking all the things to trap and normal cells
{
	while (!CellEmpty())
	{
		PickThing();
...
void Robot::puttrap()//creating traps
{
	if(bag>=5)
	{
		int a=0;
...
{
if (robot1.GetXCoordinate() == xCor + 1)
// if robot1 moves to right
{
if (robot2.GetYCoordinate() < robot1.GetYCoordinate())
// if robot2 is under the robot1
{
robot2.TurnFace(north);
robot2.Move();
}
else if (robot2.GetYCoordinate() == robot1.GetYCoordinate())
// if robot2 is in left cell of the robot1
// if robot2 is in the right cell then robot1 does not go the rigth
{
robot2.TurnFace(east);
robot2.Move();
}
else if (robot2.GetYCoordinate() > robot1.GetYCoordinate())
// if robot2 is in the up cell of the robot1
{
robot2.TurnFace(south);
robot2.Move();
}
}
else if (robot1.GetXCoordinate() == xCor - 1)
...
else if (robot1.GetYCoordinate() == yCor + 1)
// if robot1 moves to up
{

if (robot2.GetXCoordinate() < robot1.GetXCoordinate())
// if robot2 is in the left cell of the robot1
{
robot2.TurnFace(east);
robot2.Move();
}
else if (robot2.GetXCoordinate() == robot1.GetXCoordinate())
// if robot2 is under the robot1
